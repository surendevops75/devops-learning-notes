...
## During a Production deployment, ArgoCD reports Sync Successful, but the Kubernetes Deployment fails because the image tag referenced in the Git repository does not exist in Amazon ECR. How would you investigate?
...
## A Jenkins pipeline succeeds until the SonarQube scan stage, where the scanner cannot connect to the SonarQube server. Other network services are working normally. How would you troubleshoot?
...
## After upgrading Amazon EKS, several applications fail because the Metrics Server is unavailable. Horizontal Pod Autoscaler no longer scales workloads. How would you investigate?
...
## During Terraform Apply, Amazon VPC resources are created successfully, but NAT Gateway creation repeatedly fails. What would you investigate?
...
## During a Production deployment, application Pods are healthy, but Prometheus reports that no application metrics are being scraped. How would you troubleshoot?
...
## A Falco alert reports that a Production container attempted to create a privileged container from inside the cluster. What would you investigate?
...
## During a Blue-Green deployment, both environments become unavailable after switching traffic. What would you investigate?
...
## Amazon RDS replication lag increases significantly after a deployment, causing stale data to appear in read-only applications. How would you investigate?
...
## During a security review, you discover that multiple CI/CD pipelines still use long-lived AWS Access Keys instead of IAM Roles or OIDC. What would you recommend?
...
## During a Production incident, monitoring shows that every platform component is healthy, but customers cannot download invoices generated by the application. How would you investigate?

### Expected Approach

Follow the complete request flow:
...
- Validate container image availability before GitOps deployments.
- Continuously monitor SonarQube availability as a critical CI/CD dependency.
- Verify Metrics Server functionality after every Kubernetes upgrade.
- Validate networking prerequisites before provisioning NAT Gateways.
- Regularly test Prometheus scraping after application deployments.
- Investigate all Falco runtime alerts as potential security incidents.
- Always validate rollback procedures before executing Blue-Green deployments.
- Monitor database replication health for applications using read replicas.
- Replace long-lived cloud credentials with temporary identities such as IAM Roles or OIDC.
- Validate complete customer-facing workflows, including object storage access and signed URL generation, before declaring a deployment successful.
